...
// Struct to store a fuzzy search match
typedef struct {
    size_t song_index;
    int score;
} MatchResult;

// Player global state structure
static struct {
    char query[256];
    int query_len;
    ...
    // Vim modal editing & Notification state
    bool vim_mode;
...
static void enable_raw_mode(void) {
    ...
    // Enter raw mode: disable echo, canonical mode, extended processing, Ctrl-S/Q flow control
    raw.c_lflag &= ~(ECHO | ICANON | IEXTEN);
    raw.c_iflag &= ~(IXON | ICRNL);
    raw.c_oflag &= ~(OPOST); // Disable output processing (all print statements will use \r\n)
    raw.c_cflag |= (CS8);
...
static int fuzzy_match(const char *target, const char *query) {
    ...
    while (*q != '\0') {
        char q_char = *q;
        if (q_char >= 'A' && q_char <= 'Z') q_char += 32; // Lowercase
        ...
        if (curr_idx == 0 || target[curr_idx - 1] == ' ' || target[curr_idx - 1] == '/' ||
            target[curr_idx - 1] == '-' || target[curr_idx - 1] == '_' || target[curr_idx - 1] == '.') {
            score += 150;
        }
...
static void update_matches(SongList *songs) {
    ...
    if (state.selected_match_index >= state.matches_count) {
        state.selected_match_index = state.matches_count > 0 ? state.matches_count - 1 : 0;
    }
}
...
static void play_song(SongList *songs, size_t index) {
    ...
    strncpy(state.current_song_path, songs->items[index], sizeof(state.current_song_path) - 1);
    
    const char *basename = get_basename(songs->items[index]);
    strncpy(state.current_song_title, basename, sizeof(state.current_song_title) - 1);
    ...
    *dest = '\0';
    ...
    snprintf(cmd, sizeof(cmd), "{\"command\": [\"loadfile\", \"%s\", \"replace\"]}\n", escaped_path);
    send_mpv_cmd(state.mpv_sock, cmd);
    
    // Explicitly unpause to guarantee immediate playback
    send_mpv_cmd(state.mpv_sock, "{\"command\": [\"set_property\", \"pause\", false]}\n");
}

// Parses JSON properties received from mpv over socket
static void parse_mpv_message(const char *line, SongList *songs) {
    if (strstr(line, "\"event\":\"end-file\"") != NULL) {
        // Continuous playback: play next song in matches list if possible, otherwise fallback to original list
        // CRITICAL: Only trigger continuous playback if the song finished naturally ("reason":"eof").
        // This avoids triggers caused by manual replacements ("reason":"stop" or "reason":"redirect").
        if (strstr(line, "\"reason\":\"eof\"") != NULL) {
            if (state.active_song_index >= 0 && songs->len > 0) {
                ...
                if (state.matches_count > 0) {
                    ...
                    if (found_idx >= 0) {
                        next_idx = state.matches[(found_idx + 1) % state.matches_count].song_index;
                    }
                }
...
static void draw_ui(SongList *songs) {
    ...
    snprintf(mode_indicator, sizeof(mode_indicator), " [VIM: %s] ", state.vim_mode ? "NORMAL" : "INSERT");
    ...
    for (int i = 0; i < list_height; i++) {
        ...
        if (idx < state.matches_count) {
            ...
            while (*t != '\0') {
                char t_char = *t;
                char q_char = *q;
                if (q_char >= 'A' && q_char <= 'Z') q_char += 32;
                char t_char_lower = t_char;
                if (t_char_lower >= 'A' && t_char_lower <= 'Z') t_char_lower += 32;
                
                if (*q != '\0' && t_char_lower == q_char) {
                    if (is_selected) {
                        printf("\033[1;36m%c\033[0m\033[7m", t_char); // Bold Cyan Highlight inside selected
                    } else {
    ...
    if (time(NULL) < state.notification_expiry) {
        ...
        if (vol_bar_filled < 0) vol_bar_filled = 0;
        if (vol_bar_filled > 8) vol_bar_filled = 8;
    ...
    if (state.vim_mode) {
        printf("\033[90m [j/k] Down/Up  [p] Play/Pause  [s] Shuffle  [r] Random  [i] Insert  [q] Quit\033[0m\033[K");
    } else {
        printf("\033[90m [Esc] Normal Mode  [Enter] Play  [Space] Pause  [[/]] Vol  [q] Quit\033[0m\033[K");
    }
...
void start_player(SongList *songs) {
    ...
    snprintf(state.mpv_socket_path, sizeof(state.mpv_socket_path), "/tmp/fzp-mpv-%d.sock", getpid());
    ...
    if (state.mpv_pid == 0) {
        ...
        snprintf(socket_arg, sizeof(socket_arg), "--input-ipc-server=%s", state.mpv_socket_path);
        ...
        char *args[] = {
            ...
            socket_arg,
            "--no-terminal",
            "--video=no",         // Completely disable video tracks (removes album art popups)
            "--vo=null",           // Completely disable video output drivers (removes album art windows)
            "--volume=60",
            NULL
        };
    ...
    while (retries > 0) {
        ...
        if (state.mpv_sock >= 0) {
            if (connect(state.mpv_sock, (struct sockaddr *)&addr, sizeof(addr)) == 0) {
                break; // Connected!
            }
            close(state.mpv_sock);
            state.mpv_sock = -1;
    ...
    if (state.mpv_sock < 0) {
        fprintf(stderr, "Error: Could not connect to background mpv IPC socket.\r\n");
        exit(EXIT_FAILURE);
    }
    ...
    send_mpv_cmd(state.mpv_sock, "{\"command\": [\"observe_property\", 1, \"time-pos\"]}\n");
    send_mpv_cmd(state.mpv_sock, "{\"command\": [\"observe_property\", 2, \"duration\"]}\n");
    send_mpv_cmd(state.mpv_sock, "{\"command\": [\"observe_property\", 3, \"pause\"]}\n");
    send_mpv_cmd(state.mpv_sock, "{\"command\": [\"observe_property\", 4, \"volume\"]}\n");
            ...
            if (read(STDIN_FILENO, &c, 1) == 1) {
                ...
                // --- VIM COMMAND MODE INPUTS ---
                    ...
                    if (c == 'q' || c == 3) { // 'q' or Ctrl-C: exit
                        ...
                        send_mpv_cmd(state.mpv_sock, "{\"command\": [\"cycle\", \"pause\"]}\n");
                        show_notification(state.is_paused ? "Resumed Playback" : "Paused Playback");
                    ...
                    } else if (c == 'i' || c == 'a' || c == '/') { // switch to Search/Insert mode
                        state.vim_mode = false;
                        show_notification("VIM INSERT MODE");
                    } else if (c == '\r' || c == '\n') { // Enter: play selected
                        if (state.matches_count > 0 && state.selected_match_index < state.matches_count) {
                            size_t orig_idx = state.matches[state.selected_match_index].song_index;
                            play_song(songs, orig_idx);
                        }
                    } else if (c == '[') { // '[': Volume down 5%
                        send_mpv_cmd(state.mpv_sock, "{\"command\": [\"add\", \"volume\", -5]}\n");
                ...
                // --- VIM SEARCH / INSERT MODE INPUTS ---
                    ...
                    if (c == 'q' || c == 3) { // 'q' or Ctrl-C: exit
                        ...
                        send_mpv_cmd(state.mpv_sock, "{\"command\": [\"add\", \"volume\", 5]}\n");
                    } else if (c >= 32 && c < 127) { // Typable alphanumeric search characters
